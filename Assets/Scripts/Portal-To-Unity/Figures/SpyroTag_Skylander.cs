...
#region FigType Info
        ...
        public async Task FetchMagicMoment0()
        {
            ...
            else
            {
                if ((byte)(block1[0x09] + 1) != block0[0x09])
                    Debug.LogWarning("Data areas out of sync. First area chosen by default");
        ...
        {
            Debug.LogWarning(SpyroTag->magicMomentRegion0.crcType3.ChangeLittleEndianValue(EndianHelper.NativeEndianness).ToString("X4"));
            Debug.LogError(CalculateCRCType3().ToString("X4"));
        }
        ...
        public async Task SetMagicMoment1()
        {
            ...
            int areaOffset = (DataArea1 == 1) ? DATA_REGION1_OFFSET : DATA_REGION0_OFFSET;
            byte[] data = GetBlockManaged(0x07);
            byte targetBlock = (byte)(areaOffset + 0x09);
        ...
        public async Task SetRemainingData1()
        {
            ...
            for (int i = 0; i < size; i++)
            {
                byte[] data = GetBlockManaged(i + 0x08);
